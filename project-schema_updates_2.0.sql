    ...
    IF(
        (
            ...
            FROM Project p
                ...
                INNER JOIN Researcher  ON New.Researcher_ID = w.Researcher_ID
        ) <>  New.R_Organization_ID
    ) THEN SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Invalid Organization ID, Researchers work only on projects for the organizations that employed them!';
END IF;
END ;;
...
FOR EACH ROW BEGIN
    ...
    */
    IF (DATEDIFF(
        ...
        , New.Date_of_Birth) NOT BETWEEN (365 * 18 + 4) AND (365 * 80 + 20))
    THEN SIGNAL SQLSTATE '45000'
